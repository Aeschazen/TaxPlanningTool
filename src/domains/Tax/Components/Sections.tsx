            ...
            {dividendYields.map(
              ({
                ...
                <Card key={id} title={`เงินปันผล #${i + 1}`} size="small" extra={<CloseOutlined onClick={() => deleteDividendYield(i)} />}>
                  ...
                  <Form.Item label="เครดิตภาษี">
                    <ExplainableMoneyDisplay amount={taxCreditAmount} onClick={() => openExplanation(taxCreditExplanation)} />
                  </Form.Item>
                  <Form.Item label="เครดิตเป็นเงินได้สุทธิ">
                    <ExplainableMoneyDisplay amount={netIncome} onClick={() => openExplanation(netIncomeExplanation)} />
                  </Form.Item>
                </Card>
              )
            )}
