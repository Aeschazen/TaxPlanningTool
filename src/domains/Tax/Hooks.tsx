...
export function useYieldIncomeSection({incomeState,accumulatedIncomeAmount,accumulatedWithheldTaxAmount,accumulatedExpenseAmount}: YieldIncomeSetup): YieldIncomeState{
  ...
  const { dividendYields, totalIncomeAmount, totalDividendYieldWithheldTaxAmount } = dividendYieldsRef.ref.current.reduce((acc, x) => {
    const taxCreditExplanation: ReactNode[] = []
    const netIncomeExplanation: ReactNode[] = []

    netIncomeExplanation.push(<p key="0">จำนวนเงินก่อนหักภาษี: {x.incomeAmount.toFixed(precision)}</p>)
    ...
    taxCreditExplanation.push(<p key="0">เครดิตภาษี (จำนวนเงินก่อนหักภาษี × อัตราภาษีนิติบุคคล) ÷ (100 - อัตราภาษีนิติบุคคล): ({x.incomeAmount.toFixed(precision)} × {x.payerIncomeTaxRate.toFixed(precision)}) ÷ (100 - {x.payerIncomeTaxRate.toFixed(precision)}) = +{taxCreditAmount.toFixed(precision)}</p>)
    netIncomeExplanation.push(<p key="1">เครดิตภาษี: +{taxCreditAmount.toFixed(precision)}</p>)

    const netIncome = x.incomeAmount.plus(taxCreditAmount)
    netIncomeExplanation.push(<p key="2">จำนวนเงินได้สุทธิ: {netIncome.toFixed(precision)}</p>)
    ...
    acc.totalDividendYieldWithheldTaxAmount = acc.totalDividendYieldWithheldTaxAmount.plus(x.constrainedWithheldTaxAmount).plus(taxCreditAmount)
...
interface DividendYield {
  ...
  taxCreditAmount: Decimal
  taxCreditExplanation: ReactNode[]
  netIncome: Decimal
  netIncomeExplanation: ReactNode[]
}
